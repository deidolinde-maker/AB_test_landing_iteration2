...
class AddressForm:
    ...
    def _is_address_context_match(
        ...
        if normalized_locality:
            locality_prefixes = ("мкр ", "п ", "п. ", "район ", "р-н ")

            def locality_core(value: str) -> str:
                value = value.strip()
            ...
            expected_locality_core = locality_core(normalized_locality)
            locality_matches = any(
                locality_core(part) == expected_locality_core for part in parts
            )
        ...
        # District names such as "Москва, Сокол" are valid context. Reject
        # only an explicit extra locality such as "г Балашиха, мкр Саввино".
        if region_part_matches:
            locality_alias_prefixes = (
                "мкр ",
                "п ",
                "п. ",
                "с ",
            ...
            return not any(
                part.startswith(locality_alias_prefixes)
                for part in parts
                if without_admin_prefix(part) != normalized_region
                and not part.endswith((" область", " край", " республика"))
            )
